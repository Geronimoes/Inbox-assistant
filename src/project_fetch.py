#!/usr/bin/env python3
"""
Project Email Archiver — retroactive backfill of project-related emails.

Fetches emails from Gmail matching project keywords and collaborators,
and archives them using the shared email_archiver module. This script is
primarily for backfilling historical emails; ongoing archiving is handled
by fetch_and_triage.py's archive step.
...
# ── Legacy state (project-export-state.json) ────────────────────────────────
# Kept for backwards compatibility and migration. New runs also update
# archive-state.json via the EmailArchiver.

def load_export_state(data_dir: Path) -> dict:
    """Load the legacy export state file (project ID → list of email IDs)."""
...
def save_export_state(data_dir: Path, state: dict) -> None:
    """Save the legacy export state file atomically."""
    state_file = data_dir / "project-export-state.json"
    payload = dict(state)
...
def main():
    ...
    if not archive_cfg.get("enabled", False):
        ...
        print("  Enable it to use project_fetch with the unified archive.")
        sys.exit(1)

    # Inject project root for EmailArchiver
    ...
    # ── Load state ───────────────────────────────────────────────────────────
    legacy_state = load_export_state(data_dir)
    ...
    for project in selected:
        ...
        # Filter out already-exported emails (check both legacy and new state)
        already_exported = set(legacy_state.get(project["id"], []))
        new_emails = [e for e in emails
                      if e["id"] not in already_exported
                      and e["id"] not in archiver.archived_ids]
        ...
        # Archive each matched email
        exported_ids = list(already_exported)
        for email in matched:
            try:
                # Build a minimal classification for project backfill
                classification = {
                    "email_id": email["id"],
                    "category": "ACTION",  # default for project emails
                    "priority": "normal",
                    "reply_language": "en",
                }

                result = archiver.archive_email(
                    email,
                ...
                print(f"  ✗ Failed to archive email {email['id']}: {e}")

        # Save both state files after each project
        if not args.dry_run:
            legacy_state[project["id"]] = exported_ids
            save_export_state(data_dir, legacy_state)
            archiver.save_state()
            archiver.regenerate_index()

    # ── Summary ──────────────────────────────────────────────────────────────
    print(f"\n✓ Done. {total_written} note(s) written.")
    if args.dry_run:
